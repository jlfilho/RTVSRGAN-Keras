        ...
        while True:
            ...
            try: 
                ...
                for img_hr in img_crops:
                    ...
                    #img_lr = cv2.cvtColor(img_lr, cv2.COLOR_RGB2YCrCb)
                    #img_hr = cv2.cvtColor(img_hr, cv2.COLOR_RGB2YCrCb)
                    ...
                    # Store images
                    if(self.channels == 1 and self.colorspace == 'YCbCr'):
                        imgs_hr.append(img_hr[:,:,:1])
                        imgs_lr.append(img_lr[:,:,:1])
                    elif (self.channels == 3 ):
                        imgs_hr.append(img_hr[:,:,:self.channels])
                        imgs_lr.append(img_lr[:,:,:self.channels])
                    else:
                        print(">> Erro {} channels with {}.".format(self.channels,self.colorspace))
                        return -1
                    #print(imgs_hr[-1].shape,imgs_lr[-1].shape)
...
def plot_test_images(model, loader, datapath_test, test_output, epoch, name='SR_', channels = 3,colorspace='RGB'):
    ...
    try:   
        ...
        for img_hr, img_lr, img_sr, img_path in zip(imgs_hr, imgs_lr, imgs_sr, test_images):
            ...
            # Conver colorspace YCrCb to RGB
            #img_hr = cv2.cvtColor(img_hr, cv2.COLOR_YCrCb2RGB)
            #img_sr = cv2.cvtColor(img_sr, cv2.COLOR_YCrCb2RGB)
            ...
            #img_bi = cv2.cvtColor(img_bi, cv2.COLOR_GRAY2RGB)
            # Images and titles
            ...
            espcn_psnr.append(psnr(img_sr,img_hr,255.))
